...
def backwards_prove_goal(frames, current_bound, summary_proof_obligation, pdr_elements):
    ...
    while True:
        ...
        previous_bound_proof_obligation = pdr_elements.check_transformability_to_violation(frames[previous_bound].get_summaries_by_symbol_dict(),
    ...
    for i in xrange(1, current_bound + 1):
        ...
        summary_proof_obligation_generalization = pdr_elements.generalize_intransformability(frames[i-1].get_summaries_by_symbol_dict(),
                                                                                summary_proof_obligation)
...
def backward_refine_frames_or_counterexample(frames, new_bound,
    ...
    while True:
        ...
        safety_proof_obligation = pdr_elements.check_summary_safety(new_frame_summaries)
        if safety_proof_obligation is None:
            logger.debug("pdr frame %d is safe", new_bound)
            return True
       
        logger.debug("Trying to block %s from frame %d", safety_proof_obligation, new_bound)
        successfully_blocked = backwards_prove_goal(frames, new_bound, safety_proof_obligation, pdr_elements)
        if not successfully_blocked:
